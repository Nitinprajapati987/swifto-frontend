      ...
      <div style={{ background: 'linear-gradient(160deg, #bcc8f0 0%, #c5d0f5 50%, #aebde8 100%)', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: '6rem 1.5rem 2rem' }}>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem 2rem', textAlign: 'left', width: '100%', maxWidth: '420px', border: '1px solid rgba(67,97,238,0.1)', boxShadow: '0 4px 24px rgba(67,97,238,0.12)', marginBottom: '1.5rem' }}>
            ...
            {[['Tracking ID', trackingId, true], ['Pickup', form.pickup, false], ['Drop', form.drop, false], ['Vehicle', form.vehicle, false], ['Date', form.date, false]].map(([label, val, highlight]) => (
          ...
          <div style={{ background: 'white', borderRadius: '20px', padding: '1.5rem 2rem', width: '100%', maxWidth: '420px', border: '2px solid #5f259f', boxShadow: '0 4px 24px rgba(95,37,159,0.15)', marginBottom: '1.5rem', textAlign: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
              ...
              <h3 style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 900, color: '#5f259f', margin: 0, fontSize: '1.2rem' }}>Pay via PhonePe</h3>
            </div>
            <p style={{ color: '#6b7280', fontSize: '0.82rem', marginBottom: '1rem' }}>Advance payment ke liye scan karo — baaki delivery pe</p>
            ...
            <p style={{ color: '#5f259f', fontWeight: 700, fontSize: '0.85rem', marginBottom: '1rem' }}>
              📱 PhonePe / Google Pay / Paytm — sab se scan ho jaayega!
            </p>
            ...
            {!paymentDone ? (
              ...
              <div style={{ background: '#f0fdf4', border: '2px solid #22c55e', borderRadius: '12px', padding: '0.85rem', color: '#16a34a', fontWeight: 700, fontSize: '0.95rem' }}>
                🎉 Payment Confirmed! Team aapse jaldi contact karegi.
              </div>
            )}
          </div>

          <p style={{ color: '#6b7280', fontSize: '0.82rem', marginBottom: '1.5rem' }}>💡 Save this Tracking ID to track your order!</p>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '2rem', boxShadow: '0 8px 48px rgba(67,97,238,0.15)', border: '1px solid rgba(67,97,238,0.08)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            <div>
              ...
              <input name="name" value={form.name} onChange={handleChange} placeholder={form.userType === 'company' ? 'Company name' : 'Your name'} style={inputStyle(errors.name)} onFocus={e => e.target.style.borderColor = '#4361ee'} onBlur={e => e.target.style.borderColor = errors.name ? '#ef4444' : '#e8eaff'} />
              {errors.name && <p style={{ color: '#ef4444', fontSize: '0.72rem', marginTop: '4px' }}>⚠️ {errors.name}</p>}
            </div>
            <div>
              ...
              {errors.phone && <p style={{ color: '#ef4444', fontSize: '0.72rem', marginTop: '4px' }}>⚠️ {errors.phone}</p>}
            </div>
          </div>

          <div style={{ marginBottom: '1rem' }}>
            <label style={labelStyle}>📍 Pickup Location *</label>
            <input name="pickup" value={form.pickup} onChange={handleChange} placeholder="Full pickup address" style={inputStyle(errors.pickup)} onFocus={e => e.target.style.borderColor = '#4361ee'} onBlur={e => e.target.style.borderColor = errors.pickup ? '#ef4444' : '#e8eaff'} />
            {errors.pickup && <p style={{ color: '#ef4444', fontSize: '0.72rem', marginTop: '4px' }}>⚠️ {errors.pickup}</p>}
          </div>

          <div style={{ marginBottom: '1rem' }}>
            <label style={labelStyle}>📦 Drop Location *</label>
            <input name="drop" value={form.drop} onChange={handleChange} placeholder="Full drop address" style={inputStyle(errors.drop)} onFocus={e => e.target.style.borderColor = '#4361ee'} onBlur={e => e.target.style.borderColor = errors.drop ? '#ef4444' : '#e8eaff'} />
            {errors.drop && <p style={{ color: '#ef4444', fontSize: '0.72rem', marginTop: '4px' }}>⚠️ {errors.drop}</p>}
          </div>

          <div style={{ marginBottom: '1rem' }}>
            <label style={labelStyle}>🚛 Vehicle Type *</label>
            <select name="vehicle" value={form.vehicle} onChange={handleChange} style={{ ...inputStyle(errors.vehicle), cursor: 'pointer', appearance: 'auto' }}>
              <option value="">Select vehicle...</option>
              <optgroup label="Light Vehicles"><option value="Two Wheeler">🛵 Two Wheeler</option><option value="Car / Sedan">🚗 Car / Sedan</option></optgroup>
              <optgroup label="Small Trucks"><option value="7 ft Truck">🚛 7 ft Truck</option><option value="10 ft Truck">🚛 10 ft Truck</option><option value="14 ft Truck">🚛 14 ft Truck</option></optgroup>
              <optgroup label="Medium Trucks"><option value="19 ft Truck">🚚 19 ft Truck</option><option value="22 ft Truck">🚚 22 ft Truck</option><option value="24 ft Truck">🚚 24 ft Truck</option></optgroup>
              <optgroup label="Heavy Trucks"><option value="28 ft Truck">🚛 28 ft Truck</option><option value="32 ft Truck">🚛 32 ft Truck</option></optgroup>
              <optgroup label="Containers"><option value="20 ft Container">🚢 20 ft Container</option><option value="40 ft Container">🚢 40 ft Container</option></optgroup>
              <optgroup label="Special"><option value="Dumper">🚜 Dumper</option><option value="Tanker">🛢️ Tanker</option><option value="Trailer">🚛 Trailer</option></optgroup>
            </select>
            {errors.vehicle && <p style={{ color: '#ef4444', fontSize: '0.72rem', marginTop: '4px' }}>⚠️ {errors.vehicle}</p>}
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            <div>
              <label style={labelStyle}>📅 Pickup Date *</label>
              <input type="date" name="date" value={form.date} onChange={handleChange} min={new Date().toISOString().split('T')[0]} style={inputStyle(errors.date)} onFocus={e => e.target.style.borderColor = '#4361ee'} onBlur={e => e.target.style.borderColor = errors.date ? '#ef4444' : '#e8eaff'} />
              {errors.date && <p style={{ color: '#ef4444', fontSize: '0.72rem', marginTop: '4px' }}>⚠️ {errors.date}</p>}
            </div>
            <div>
              <label style={labelStyle}>⚖️ Approx Weight</label>
              <input name="weight" value={form.weight} onChange={handleChange} placeholder="e.g. 500 kg, 2 ton" style={inputStyle(false)} onFocus={e => e.target.style.borderColor = '#4361ee'} onBlur={e => e.target.style.borderColor = '#e8eaff'} />
            </div>
          </div>

          <div style={{ marginBottom: '1.25rem' }}>
            <label style={labelStyle}>📝 Goods Description</label>
            <textarea name="goods" value={form.goods} onChange={handleChange} placeholder="What are you shipping — furniture, machinery, FMCG..." rows={3} style={{ ...inputStyle(false), resize: 'none' }} onFocus={e => e.target.style.borderColor = '#4361ee'} onBlur={e => e.target.style.borderColor = '#e8eaff'} />
          </div>
          ...
          <div style={{ background: '#eef0ff', border: '1px solid rgba(67,97,238,0.2)', borderRadius: '12px', padding: '1rem', fontSize: '0.82rem', color: '#3451d1', marginBottom: '1.25rem' }}>
            💡 <strong>Note:</strong> After submitting, our team will call you within 30 minutes to confirm the final price.
          </div>
          ...
          <button onClick={handleSubmit} disabled={loading}
            ...
            {loading ? (<><svg style={{ animation: 'spin 1s linear infinite', width: '18px', height: '18px' }} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="white" strokeWidth="3" strokeOpacity="0.3"/><path d="M12 2a10 10 0 0110 10" stroke="white" strokeWidth="3" strokeLinecap="round"/></svg>Processing...</>) : '🚀 Submit Booking'}
          </button>
        </div>
      </div>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Manrope:wght@700;800;900&family=DM+Sans:wght@400;500;600&display=swap'); @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } } input::placeholder, textarea::placeholder { color: #9ca3af; }`}</style>
    </div>
  );
}
