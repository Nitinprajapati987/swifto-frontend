    ...
    <div style={{ background: 'linear-gradient(160deg, #bcc8f0 0%, #c5d0f5 50%, #aebde8 100%)', minHeight: '100vh', paddingBottom: '3rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '7rem 1.5rem 2rem', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <h2 style={{ fontFamily: 'Manrope, sans-serif', fontSize: '2.5rem', fontWeight: 900, color: '#1a1a2e', letterSpacing: '-1px', marginBottom: '0.5rem' }}>Track Your <span style={{ color: '#4361ee' }}>Order</span></h2>
        ...
        {data && booking && (
          <div style={{ background: 'white', borderRadius: '20px', padding: '1.75rem', boxShadow: '0 8px 48px rgba(67,97,238,0.15)', border: '1px solid rgba(67,97,238,0.08)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem', paddingBottom: '1.25rem', borderBottom: '1px solid #e8eaff', flexWrap: 'wrap', gap: '0.75rem' }}>
              <div><p style={{ fontSize: '0.7rem', color: '#9ca3af' }}>Tracking ID</p><p style={{ fontWeight: 800, color: '#4361ee', fontFamily: 'Manrope, sans-serif' }}>{data.trackingId}</p></div>
              <div><p style={{ fontSize: '0.7rem', color: '#9ca3af' }}>Vehicle</p><p style={{ fontWeight: 700, color: '#1a1a2e', fontSize: '0.9rem' }}>{booking.vehicle}</p></div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', background: '#f5f7ff', borderRadius: '12px', padding: '1rem', border: '1px solid #e8eaff' }}>
              <div><p style={{ fontSize: '0.7rem', color: '#9ca3af', marginBottom: '2px' }}>From</p><p style={{ fontWeight: 800, fontSize: '0.95rem', color: '#1a1a2e' }}>{booking.pickup}</p></div>
              <div style={{ textAlign: 'center' }}><div style={{ color: '#4361ee', fontSize: '1.25rem' }}>→</div></div>
              <div style={{ textAlign: 'right' }}><p style={{ fontSize: '0.7rem', color: '#9ca3af', marginBottom: '2px' }}>To</p><p style={{ fontWeight: 800, fontSize: '0.95rem', color: '#1a1a2e' }}>{booking.drop}</p></div>
            </div>

            <div style={{ marginBottom: '1.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: '#6b7280', marginBottom: '8px' }}>
                <span>Progress</span><span style={{ color: '#4361ee', fontWeight: 700 }}>{progress}% Complete</span>
              </div>
            ...
            <div style={{ borderTop: '1px solid #e8eaff', paddingTop: '1.25rem' }}>
              <Link to="/booking" style={{ display: 'block' }}>
                <button style={{ width: '100%', background: '#4361ee', color: 'white', padding: '0.75rem', borderRadius: '10px', fontSize: '0.85rem', fontWeight: 700, border: 'none', cursor: 'pointer', boxShadow: '0 2px 8px rgba(67,97,238,0.25)' }}>New Booking</button>
              </Link>
            </div>
          </div>
        )}
